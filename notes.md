...
Las coordenadas Jacobianas representan (x,y) como (X:Y:Z) con x=X/Z², y=Y/Z³. La teoría clásica dice que una inversión `I` cuesta ~**80-100 multiplicaciones `M`**. Como Jacobianas eliminan la inversión por paso (la difieren a una sola al final de la mult. escalar — de ~384 inversiones a 1), un conteo ingenuo predice hasta ~8×.

**Pero medimos solo 1,7×-2×.** ¿Por qué? Porque la rutina de inversión de **NTL/GMP está extremadamente optimizada** (Euclides extendido / GCD binario en ensamblador). El cociente real I/M aquí es mucho menor que 80-100, así que quitar inversiones ahorra menos de lo previsto. La aceleración crece con el cuerpo (1,72× en 256 bits, ~2× en P-384) porque la inversión se encarece más rápido que la multiplicación al crecer el primo.

> Lección de defensa: "El conteo teórico de operaciones es una guía, pero las características de bajo nivel de la librería aritmética pueden cambiar el resultado real. Es uno de los hallazgos más ilustrativos del trabajo."
...
## 8.4 ECDSA binaria no implementada

- El módulo `ecc_binary` solo implementa KeyGen, mult. escalar y ECDH. La figura 7.14 lo confirma (sin columnas Sign/Verify). Respuesta: no, solo ECDH.
