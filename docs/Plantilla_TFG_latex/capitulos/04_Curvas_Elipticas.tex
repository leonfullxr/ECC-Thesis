...
\begin{enumerate}
  ...
  \item El punto \(\O\) es el único punto de la ``línea en el infinito'' que satisface la forma proyectiva de la ecuación de Weierstrass.
  \item Los \emph{puntos \(L\)-racionales} de \(E\) son aquellos cuyos pares \((x,y)\) pertenecen al cuerpo \(L\). Siempre incluimos \(\O\).
\end{enumerate}
...
\[
  ...
  E_2: \;y^2 = x^3 + 3x + 15.
\]
Las gráficas de los conjuntos \(E_1(\mathbb{R})\setminus\{\O\}\) y
\(E_2(\mathbb{R})\setminus\{\O\}\) se muestran en la figura~\ref{fig:curvas_reales}.
\end{ejemplo}
...
\begin{figure}[H]
  ...
  \caption{Curvas \(E_1\) y \(E_2\) sobre \(\mathbb{R}\), con sus respectivos puntos (excepto \(\O\)).}
...
Sea \(E\) una curva elíptica definida sobre un cuerpo \(K\). El conjunto de puntos \(E(K)\), junto con un elemento especial \(\O\) (punto en el infinito), adquiere una estructura de grupo abeliano mediante la llamada \emph{regla de la cuerda y la tangente}. A continuación describimos primero la construcción geométrica y, después, distinguimos los casos algebraicos según la posición relativa de los dos operandos.

\subsubsection*{Descripción geométrica}
\begin{itemize}
  \item \textbf{Identidad.} El punto en el infinito \(\O\) actúa como elemento neutro:
    \[
      P + \O \;=\; \O + P \;=\; P,\qquad\forall\,P\in E(K).
    \]
  \item \textbf{Opuesto.} Dado \(P=(x,y)\in E(K)\), su opuesto es \(-P=(x,-y)\) (la reflexión sobre el eje \(x\)), de modo que \(P + (-P) = \O\).
...
\begin{enumerate}
  \item \textbf{Caso 1: alguno de los operandos es \(\O\).} Si \(P=\O\), entonces \(R = Q\); análogamente si \(Q=\O\), entonces \(R = P\).
