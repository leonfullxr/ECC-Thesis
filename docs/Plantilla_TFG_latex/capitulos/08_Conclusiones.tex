...
\paragraph{El nivel de optimización del compilador tiene un efecto acotado.}
El estudio de las distintas opciones de compilación reveló que la mejora de rendimiento se concentra en el salto de \texttt{-O0} a los niveles \texttt{-O1}/\texttt{-O2}, mientras que las opciones más agresivas (\texttt{-O3}, \texttt{-Ofast}, \texttt{-march=native}) aportan ganancias marginales. La razón es estructural: el núcleo aritmético reside en GMP, que se enlaza ya compilado con su propio ensamblador optimizado y, por tanto, queda fuera del alcance de los \textit{flags} del proyecto. Este hallazgo justifica de forma empírica la elección de \texttt{-O2} como nivel de compilación: alcanza casi todo el rendimiento posible sin la imprevisibilidad de las transformaciones más agresivas ni la pérdida de portabilidad asociada a \texttt{-march=native}.
...
\begin{enumerate}
    ...
    \item \textbf{Alcance de las mediciones.} Los benchmarks se ejecutaron con 100 iteraciones por operación sobre una única máquina. Aunque suficiente para apreciar las tendencias, un número mayor de iteraciones y la ejecución en varias plataformas reforzarían la robustez estadística, especialmente para la generación de claves de RSA, de varianza elevada.
