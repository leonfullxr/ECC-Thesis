\chapter{Cuerpos finitos}\label{chap:Campos_finitos}
...
Los cuerpos finitos (también llamados \emph{cuerpos de Galois}) se denotan como \(\mathrm{GF}(p^m)\), donde \(p\) es un número primo y \(m\) un entero positivo. Para que los sistemas de curvas elípticas sean eficientes, es imprescindible implementar de manera óptima la suma, la resta, la multiplicación y la inversión en dicho cuerpo. Existen tres familias de cuerpos que resultan especialmente adecuadas para ECC (Criptografía de Curvas Elípticas):  
\begin{itemize}
  \item \textbf{Cuerpos primos} (\(\mathbb{F}_p\)),  
  \item \textbf{Cuerpos binarios} (\(\mathbb{F}_{2^m}\)),  
  \item \textbf{Cuerpos de extensión óptima} (OEF).  
\end{itemize}
...
A lo largo de este trabajo emplearemos indistintamente las notaciones \(\mathrm{GF}(p^m)\) y \(\mathbb{F}_{p^m}\) para referirnos al cuerpo finito de orden \(p^m\); ambas son habituales en la literatura.

\section{Introducción a cuerpos finitos}
...
El \emph{orden} de un cuerpo finito es el número de sus elementos. Existe un cuerpo finito de orden \(q\) si y solo si \(q\) es una potencia prima, es decir, \(q = p^m\) con \(p\) primo y \(m\) entero positivo.

\begin{itemize}
  \item Si \(m=1\), el cuerpo se llama \emph{cuerpo primo} y se denota \(\mathbb{F}_p\).
  \item Si \(m\ge2\), se llama \emph{cuerpo de extensión} y se denota \(\mathbb{F}_{p^m}\).
\end{itemize}
...
\begin{definicion}[Característica de un cuerpo]\label{def:caracteristica}
La \emph{característica} de un cuerpo \(K\), denotada \(\mathrm{char}(K)\), es el menor entero positivo \(n\) tal que
\[
...
\begin{ejemplo}
Los cuerpos \(\mathbb{Q}\), \(\mathbb{R}\) y \(\mathbb{C}\) tienen característica \(0\). El cuerpo finito \(\mathbb{F}_q\) con \(q = p^m\) tiene característica \(p\); en particular, en \(\mathbb{F}_p\) se cumple que \(p\cdot a = 0\) para todo \(a\in\mathbb{F}_p\).
\end{ejemplo}

\begin{remark}
La característica del cuerpo base es decisiva en la teoría de curvas elípticas: las fórmulas para suma y duplicación de puntos, y la propia ecuación simplificada de Weierstrass, dependen del valor de \(\mathrm{char}(K)\) (véanse las secciones~\ref{sec:simplificaciones_weierstrass_curvas_elipticas} y~\ref{sec:ley_de_grupo}). En particular, los casos \(\mathrm{char}(K)\in\{2,3\}\) requieren un tratamiento separado, porque no se puede dividir entre \(2\) o entre \(3\) en estos cuerpos.
\end{remark}
...
Para sumar dos enteros en binario aplicamos el método enseñado en primaria, sumando uno a uno (bit a bit) y propagando el acarreo (la \emph{llevada}) hacia la siguiente posición cuando sea necesario. Por ejemplo, para calcular \(43+37=80\) en binario, donde \(43 = 101011_2\) y \(37 = 100101_2\):
\[
...
\end{array}
\]
Los acarreos que entran en cada posición aparecen en rojo en la fila superior. El resultado es \(1010000_2 = 80\).
...
\label{sec:aritmetica-fp}
Sea $p$ un número primo. El conjunto
$$
  \mathbb{F}_p = \{0,1,2,\dots,p-1\},
$$
con las operaciones de suma y multiplicación módulo $p$, $\mathbb{Z}/p\mathbb{Z}$, constituye un cuerpo finito de orden $p$. Denotaremos este cuerpo por $\mathbb{F}_p$ y llamaremos a $p$ el \emph{módulo} de $\mathbb{F}_p$. Para cualquier entero $a$, $a \bmod p$ denota el único residuo $r$, $0\le r\le p-1$, obtenido al dividir $a$ por $p$; esta operación se llama \emph{reducción módulo $p$}.
...
\begin{ejemplo}[Cuerpo primo $\mathbb{F}_{31}$]\label{ej:F31}
...
\subsection{Suma en \texorpdfstring{$\mathbb{F}_p$}{Fp}}
Para sumar dos elementos de \(\mathbb{F}_p\cong\mathbb{Z}/p\mathbb{Z}\), representados como enteros únicos en \([0,p-1]\), basta sumar los enteros y comprobar si el resultado es \(\ge p\); en tal caso restamos \(p\) para obtener un valor en \([0,p-1]\). De forma análoga, tras restar dos enteros, sumamos \(p\) si el resultado es negativo. El coste total sigue siendo \(O(n)\) operaciones a nivel de bit, donde \(n=\lg p\)\footnote{A lo largo de la memoria, \(\lg\) denota el logaritmo en base 2, es decir, \(\lg x = \log_2 x\).} es el número de bits necesarios para representar un elemento del cuerpo.
...
Calculemos \(43\times37=1591\) con el método de la escuela en binario. Como \(37 = 100101_2\) tiene tres bits activos (en las posiciones \(0\), \(2\) y \(5\)), el producto se obtiene como suma de tres copias desplazadas de \(43 = 101011_2\):
\[
\begin{array}{l@{\;\;}*{11}{c}}
       &   &   &   &   &   & 1 & 0 & 1 & 0 & 1 & 1 \\
\times &   &   &   &   &   & 1 & 0 & 0 & 1 & 0 & 1 \\
...
\label{sec:aritmetica-f2m}
Los cuerpos finitos de orden $2^m$, también llamados \emph{cuerpos binarios}, se construyen como extensiones de polinomios:
\[
  \mathbb{F}_{2^m}\cong\mathbb{F}_2[x]/(f(x)),
\]
donde $f(x)$ es un polinomio irreducible de grado $m$ sobre $\mathbb{F}_2$.
En tal cuerpo, cada elemento puede representarse como un polinomio de grado \(m\) < m con coeficientes en ${0,1}$ (equivalentemente, como un vector binario de $m$ bits).

\subsection{Representación polinomial}
Sea \(f(z)\) un polinomio binario irreducible de grado \(m\), se puede escribir como
\[
  f(z) = z^m + r(z).
\]
...
\[
  a(z)\cdot b(z)\bmod f(z).
\]

\begin{ejemplo}[Cuerpo $\mathbb{F}_{2^3}$]\label{ej:F8}
...
\begin{ejemplo}[Isomorfismo de cuerpos]\label{ej:isomorfismo_F8}
Para $m=3$ hay dos polinomios irreducibles sobre $\mathbb{F}_2$: $f_1(x)=x^3+x+1$ y $f_2(x)=x^3+x^2+1$. Los cuerpos $\mathbb{F}_2[x]/(f_1)$ y $\mathbb{F}_2[x]/(f_2)$ tienen los mismos elementos como conjunto y son isomorfos: existe un mapeo que envía la clase de $x$ en uno a una raíz de $f_1$ en el otro.
\end{ejemplo}
...
\For{\(i = 1\) \textbf{ to } \(m-1\)}
  ...
  \If{el bit desplazado era \(1\)}
    \State \(B \gets B \oplus R\)
  \EndIf
  \If{\(a_i = 1\)}
    \State \(C \gets C \oplus B\)
  \EndIf
\EndFor
\State \Return \(C\)
\end{algorithmic}
...
\begin{remark}
Los OEF se incluyen aquí por completitud teórica y para enmarcar la elección de cuerpos en ECC. La implementación desarrollada en el capítulo~\ref{ch:implementacion} no hace uso de OEF: se limita a $\mathbb{F}_p$ y $\mathbb{F}_{2^m}$, que son las dos familias estandarizadas para criptografía de curvas elípticas (NIST, SECG). El material restante de esta sección se ofrece como referencia.
\end{remark}
...
La idea de los \emph{cuerpos de extensión óptima} (OEF) es elegir \(p\), \(m\) y el polinomio de reducción de forma que se ajusten mejor al hardware; en concreto, escoger \(p\) de modo que quepa en una sola palabra simplifica el manejo del acarreo.

\begin{definicion}
Un \emph{cuerpo de extensión óptima} (OEF) es un cuerpo finito \(\mathbb{F}_{p^m}\) tal que:
\begin{enumerate}
...
\section{Selección de cuerpos para ECC: comparativa y recomendaciones}
\label{sec:seleccion-cuerpos-ecc}
...
\begin{itemize}
  \item \textbf{Cuerpos primos \(\mathbb{F}_p\):}  
    \begin{itemize}
      \item Muy usados en estándares (p.ej. NIST P-256, SECP256k1)~\citep{nist-fips186-4}.
      \item Operaciones de multiplicación y reducción pueden aprovechar multiplicadores de enteros en hardware~\citep{hankerson2004guide}.  
      \item Más sencillos de proteger frente a canales laterales (menos ramificaciones en tiempo de reducción)~\citep{kocher1999dpa}.
    \end{itemize}
  \item \textbf{Cuerpos binarios \(\mathbb{F}_{2^m}\):}  
    \begin{itemize}
      \item Ventajosos en hardware ligero (solo desplazamientos y XOR)~\citep{sec2-v2}.
      \item Menos populares en software general-propósito porque requieren multiplicación bit-a-bit~\citep{hankerson2004guide}.  
      \item Históricamente usados en tarjetas inteligentes y estándares ISO/IEC~\citep{sec2-v2}.  
    \end{itemize}
  \item \textbf{Cuerpos de extensión óptima (OEF):}  
    \begin{itemize}
      ...
      \item Para aplicaciones de alto rendimiento, se emplean de Tipo I o II cuyas operaciones en \(\mathbb{F}_p\) caben en una palabra de máquina y el polinomio de reducción es un binomio simple \((z^m-\omega)\)~\citep{militzer2004oef,barreto2003oef}. Ejemplos concretos se recogen en la Tabla \ref{tab:oef_params}.
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
  \item \emph{Rendimiento}: mediante tiempos de multiplicación e inversión. En CPU modernas, usar primos especiales (Mersenne, pseudo-Mersenne) acelera la reducción\citep{bernstein2006curve25519}.
  \item \emph{Resistencia a canales laterales}: operaciones condicionales mínimas, tiempo de ejecución constante y operaciones atómicas~\citep{kocher1999dpa}.
  \item \emph{Compatibilidad y estandarización}: preferir curvas en \(\mathbb{F}_p\) ampliamente auditadas (p.ej. curvas NIST, Brainpool, Curve25519)~\citep{ietf-cfrg}.
  \item \emph{Restricciones de hardware}: en dispositivos con ALU reducida puede convenir \(\mathbb{F}_{2^m}\) o OEF para evitar multiplicadores de enteros avanzados~\citep{militzer2004oef}.
\end{enumerate}

\subsection*{Comparativa orientativa de costes}

A modo de referencia, el cuadro~\ref{tab:comparativa_campos} reproduce órdenes de magnitud típicos extraídos de la literatura~\citep{openssl-bench,hankerson2004guide}; los tiempos exactos dependen del hardware, la versión de la biblioteca y el nivel de optimización. Una comparativa empírica obtenida mediante mediciones propias se presenta en el capítulo~\ref{ch:implementacion}.

\begin{table}[H]
  \centering
  \begin{tabular}{lccc}
    \hline
    \textbf{Cuerpo} & \textbf{Multiplicación} & \textbf{Inversión} & \textbf{Uso típico} \\
    \hline
    ...
    \hline
  \end{tabular}
  \caption{Órdenes de magnitud orientativos de los costes en una CPU de 3 GHz para cuerpos de 256 bits, según la literatura. Los valores experimentales medidos sobre la implementación de este trabajo se presentan en el capítulo~\ref{ch:implementacion}.}
