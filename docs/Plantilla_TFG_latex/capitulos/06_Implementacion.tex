...
\begin{description}
    ...
    \item[Valor académico.] La progresión afines $\to$ Jacobianas es en sí misma un resultado medible del TFG: el speedup observado (3--5$\times$ en la práctica, frente al teórico 8.3$\times$) constituye un dato experimental propio.
\end{description}
...
\hline
\multirow{6}{*}{RSA} & Keygen & Generación de claves (1024, 2048, 3072, 4096 bits) \\
...
\hline
\multirow{6}{*}{ECC (afín)} & Keygen & Generación $Q = dG$ (P-256, P-384, secp256k1) \\
...
\hline
\multirow{3}{*}{ECC (Jacobiano)} & Keygen & Igual que afín, con \texttt{use\_jacobian=true} \\
 & ECDH & Secreto compartido con aritmética Jacobiana \\
 & ECDSA sign/verify & Firma y verificación con aritmética Jacobiana \\
\hline
\multirow{3}{*}{ECC (binario)} & Keygen & Generación sobre $\mathbb{F}_{2^m}$ (5 curvas SEC~2) \\
...
Todas las comparaciones RSA-ECC se realizan a niveles de seguridad equivalentes según NIST SP~800-57: RSA-3072 frente a P-256/secp256k1 (128 bits de seguridad) y RSA-7680 frente a P-384 (192 bits).
...
\begin{enumerate}
    ...
    \item \textbf{Validación incompleta de parámetros.} Como se detalla en la sección~\ref{subsec:impl-validacion-params}, no se verifica la primalidad de~$n$, $nG = \O$, ni las condiciones de inmersión y anomalía, confiando en los parámetros estandarizados.
\end{enumerate}
