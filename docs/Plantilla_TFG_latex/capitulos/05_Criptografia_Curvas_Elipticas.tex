...
\paragraph{El método rho de Pollard para el ECDLP.}

De los dos algoritmos genéricos mencionados, el \emph{rho de Pollard}~\cite{pollard1978monte} es el más utilizado en la práctica contra el ECDLP, pues tiene la misma complejidad temporal que baby-step/giant-step ($O(\sqrt{n})$) pero con requerimiento de memoria constante~$O(1)$, lo que lo hace viable para atacar curvas de tamaños criptográficos.

\subparagraph{Idea intuitiva.}
El método se basa en la \emph{paradoja del cumpleaños}: si elegimos elementos al azar de un conjunto de~$N$ elementos, esperamos encontrar una colisión (dos elementos iguales) tras aproximadamente~$\sqrt{\pi N/2}$ elecciones. Pollard aplica este principio al grupo de puntos de la curva: en lugar de elegir puntos al azar (lo cual requeriría almacenarlos todos), construye una \emph{secuencia pseudoaleatoria} de puntos mediante una función de iteración determinista. Cuando la secuencia produce una colisión ---dos puntos iguales---, se puede extraer el logaritmo discreto.

\subparagraph{Descripción del algoritmo.}
Sea $P$ el generador de orden~$n$ y $Q = dP$ el punto cuyo logaritmo discreto queremos encontrar. Definimos una función de iteración $f: \langle P \rangle \to \langle P \rangle$ que particiona el grupo en tres subconjuntos $S_1, S_2, S_3$ (por ejemplo, según el valor de~$x \bmod 3$) y actúa como:
\[
    f(R) = \begin{cases}
        Q + R & \text{si } R \in S_1, \\
        2R    & \text{si } R \in S_2, \\
...
Se mantienen dos secuencias simultáneas partiendo de un punto inicial $R_0 = a_0 P + b_0 Q$ con $a_0, b_0$ conocidos:
\begin{itemize}
    \item La secuencia ``lenta'': $R_i = f(R_{i-1})$.
    \item La secuencia ``rápida'': $R_i' = f(f(R_{i-1}'))$ (aplica $f$ dos veces por paso).
\end{itemize}

En cada paso se actualizan los coeficientes $a_i, b_i$ tales que $R_i = a_i P + b_i Q \pmod{n}$. Dado que el grupo es finito, eventualmente ambas secuencias coincidirán en un punto: $R_i = R_j'$, lo cual da
\[
    a_i P + b_i Q = a_j P + b_j Q
    \quad\Longrightarrow\quad
    (a_i - a_j) P = (b_j - b_i) Q = (b_j - b_i) d P,
\]
y si $\gcd(b_j - b_i,\, n) = 1$, entonces
\[
    d = \frac{a_i - a_j}{b_j - b_i} \pmod{n}.
\]

Este truco de la ``tortuga y la liebre'' (detección de ciclos de Floyd) evita almacenar toda la secuencia: basta con mantener dos puntos en memoria.

\subparagraph{Complejidad y significado práctico.}
El número esperado de iteraciones hasta encontrar una colisión es $O(\sqrt{n})$, donde cada iteración consiste en una o dos operaciones de grupo (sumas y duplicaciones de puntos). Para una curva de 256~bits con $n \approx 2^{256}$, esto supone $\approx 2^{128}$ operaciones, lo que está fuera del alcance de cualquier capacidad computacional actual o previsible a medio plazo.

El récord de cómputo para el ECDLP fue establecido por Certicom en el contexto de su \emph{ECC Challenge}: la curva ECC2K-130 (130~bits sobre campo binario) fue resuelta en 2009 tras un esfuerzo computacional masivo equivalente a más de un año de cálculo distribuido~\cite{gaudry2007breaking}. Esto confirma experimentalmente que para curvas de 256~bits, el método rho de Pollard es completamente inviable.

\begin{observacion}
    Es importante destacar que el rho de Pollard es un \emph{algoritmo genérico}: no explota ninguna estructura específica de las curvas elípticas, solo la estructura de grupo finito. Esto significa que, a diferencia de los ataques \textit{index calculus} contra el DLP en~$\mathbb{F}_p^*$, no se conoce forma de ``transferir'' las propiedades aritméticas del campo subyacente para acelerar el ataque en curvas elípticas. Esta es la razón fundamental por la que ECC ofrece mayor seguridad por bit que RSA.
\end{observacion}
...
El protocolo de Diffie--Hellman sobre curvas elípticas (ECDH) es la adaptación directa del intercambio de claves Diffie--Hellman clásico (sección~\ref{subsec:dh-clasico}) al grupo de puntos de una curva elíptica. Su seguridad se fundamenta en el \emph{problema de Diffie--Hellman computacional en curvas elípticas} (ECDHP): dados $P$, $aP$ y $bP$, calcular $abP$.
