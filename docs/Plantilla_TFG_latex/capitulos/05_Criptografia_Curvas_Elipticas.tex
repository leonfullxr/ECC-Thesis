...
\begin{itemize}
    ...
    \item $n$ es el \emph{orden} de $G$, es decir, el menor entero positivo tal que $n \cdot G = \O$.
...
\State Verificar que $G \neq \O$ y que $G$ está en la curva: $G_y^2 \equiv G_x^3 + aG_x + b \pmod{p}$.
\State Verificar que $n$ es primo.
\State Verificar que $n \cdot G = \O$. \Comment{$G$ tiene orden $n$}
...
\begin{enumerate}
    \item $Q \neq \O$ (no es el punto en el infinito).
    \item $Q$ pertenece a la curva: $Q_y^2 \equiv Q_x^3 + aQ_x + b \pmod{p}$.
    \item $Q$ pertenece al subgrupo correcto: $n \cdot Q = \O$.
\end{enumerate}
...
\begin{enumerate}
    ...
    \]
    \item Si $R' = \O$, rechazar. En caso contrario, calcular $v = x_{R'} \bmod n$.
...
\Ensure $Q = k \cdot P$
\State $Q \leftarrow \O$ \Comment{Punto en el infinito}
