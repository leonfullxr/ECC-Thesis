...
\begin{frame}{¿Qué es una curva elíptica?}
  \footnotesize
  Soluciones de la \alert{ecuación de Weierstrass} $y^2 = x^3 + ax + b$
  (con char$(K)\neq 2,3$), más el \alert{punto del infinito} $\mathcal{O}$.\;
  \alert{No singular} $\iff \Delta = -16(4a^3 + 27b^2) \neq 0$
  (sin cúspides ni autointersecciones): es la condición para que la
  \alert{ley de grupo} esté bien definida.
  \begin{center}
  ...
  27 por char != 2,3. Tercer panel: en un punto singular la tangente no es unica y 2P divide
  por 2y=0, por eso la ley de grupo falla.}
\end{frame}
