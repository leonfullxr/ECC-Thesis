...
def main():
    ...
    y_max_real = np.nanmax(y_up[finite]) if finite.any() else 1.0
    R = max(abs(x_min), abs(x_max), y_max_real * 1.1)
    plot_min, plot_max = -R, R

    fig, ax = plt.subplots(figsize=(6,6))

    # 1) draw center axes first
    ax.axvline(0, color="black", linestyle="--", linewidth=1, zorder=1)
    ax.axhline(0, color="black", linestyle="--", linewidth=1, zorder=1)

    # 2) plot the elliptic curve
    curve_color = "blue"
    ax.plot(x, y_up, color=curve_color, linewidth=2, zorder=2, label="Curve")
    ax.plot(x, y_dn, color=curve_color, linewidth=2, zorder=2)

    # 3) if P and Q provided, visualize addition
    if args.P and args.Q:
        x1, y1 = args.P
        x2, y2 = args.Q
        same_point = np.isclose(x1, x2) and np.isclose(y1, y2)
        skip_sum = False

        # compute slope m or handle vertical tangent on doubling
        if same_point:
            if np.isclose(y1, 0.0):
                # P = Q is a 2-torsion point: vertical tangent
                xs_line = np.full(300, x1)
                ys_line = np.linspace(plot_min, plot_max, 300)
                ax.plot(xs_line, ys_line,
                        color="green", linestyle="-.", linewidth=1.5,
                        zorder=3, label="Vertical tangent")
                skip_sum = True
            else:
                m = (3*x1**2 + a) / (2*y1)
        else:
        ...
        if not skip_sum:
            ...
            xs_line = np.linspace(min(x1, x2, x3)-1,
                                  max(x1, x2, x3)+1, 300)
            ys_line = m*xs_line + c
            ax.plot(xs_line, ys_line,
                    color="green", linestyle="-.", linewidth=1.5,
        ...
        # plot P (and Q, only if distinct)
        if same_point:
            ax.scatter(x1, y1, color="red", s=60, zorder=4,
                       label="P = Q")
            ax.annotate("P=Q", (x1, y1),
        ...
        if not skip_sum:
            ax.scatter(xS, yS, color="orange", s=60, zorder=4,
                       label="S = intersection")
            ax.annotate("S", (xS, yS),
                        textcoords="offset points", xytext=(5,5))
            ax.scatter(xR, yR, color="magenta", s=60, zorder=4,
                       label="R = P+Q")
            ax.annotate("R", (xR, yR),
